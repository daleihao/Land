...
can3 = CanopyLayer{FT}(n_leaf=1);
can4 = CanopyLayer{FT}(n_leaf=1);
...
modC3 = C3CLM(FT)
stoC3 = ESMBallBerry{FT}(g1 = 16)

## C4 Photosynthesis
modC4 = C4CLM(FT)
stoC4 = ESMBallBerry{FT}(g1 = 8)
#----------------------------------------------------------------------------
...
for _APAR in APAR
    ...
    leaf_photo_from_envir!(modC3, can3, lhs, envir, stoC3);
    ## Save leaf variables:
    push!(An_C3, can3.An[1]); push!(Ag_C3, can3.Ag[1]);
    push!(Aj_C3, can3.Aj[1]); push!(Ap_C3, can3.Ap[1]);
    push!(Ac_C3, can3.Ac[1]); push!(Cc_C3, can3.p_i[1]);
    push!(gs_C3, can3.g_sw[1]);

    ## Run C4 photosynthesis:
    leaf_photo_from_envir!(modC4, can4, lhs, envir, stoC4);
    ## Save leaf variables:
    push!(An_C4, can4.An[1]); push!(Ag_C4, can4.Ag[1]);
    push!(Aj_C4, can4.Aj[1]); push!(Ap_C4, can4.Ap[1]);
    push!(Ac_C4, can4.Ac[1]); push!(Cc_C4, can4.p_i[1]);
    push!(gs_C4, can4.g_sw[1]);
end
#----------------------------------------------------------------------------
...
for iA in eachindex(APAR), iC in eachindex(CO2), iT in eachindex(T)
    ...
    ## Save leaf variables:
    An_C3[iA,iC,iT]=can3.An[1];
    Ag_C3[iA,iC,iT]=can3.Ag[1];
    Aj_C3[iA,iC,iT]=can3.Aj[1];
    Ap_C3[iA,iC,iT]=can3.Ap[1];
    Ac_C3[iA,iC,iT]=can3.Ac[1];
    Cc_C3[iA,iC,iT]=can3.p_i[1];
    gs_C3[iA,iC,iT]=can3.g_sw[1];

    ## Run C4 photosynthesis:
    leaf_photo_from_envir!(modC4, can4, lhs, envir, stoC4);

    ## Save leaf variables:
    An_C4[iA,iC,iT]=can4.An[1];
    Ag_C4[iA,iC,iT]=can4.Ag[1];
    Aj_C4[iA,iC,iT]=can4.Aj[1];
    Ap_C4[iA,iC,iT]=can4.Ap[1];
    Ac_C4[iA,iC,iT]=can4.Ac[1];
    Cc_C4[iA,iC,iT]=can4.p_i[1];
    gs_C4[iA,iC,iT]=can4.g_sw[1];
end
#----------------------------------------------------------------------------
